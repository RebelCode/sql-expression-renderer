...
use RebelCode\Expression\Renderer\Sql\SqlGenericFunctionExpressionTemplate as TestSubject;
use Xpmock\TestCase;
 ...
 * @since [*next-version*]
 */
class SqlGenericFunctionExpressionTemplateTest extends TestCase
{
    /**
    ...
    public function testRender()
    {
        ...
        $expression = $this->createExpression($type, $terms);
        $ctx = [
        ...
        $subject = new TestSubject($dlgTemplate);

        $expected = "$fnType($render1, $render2)";
    ...
    /**
     * Tests the render method with an "sql_" prefixed type to ensure that the expression is correctly rendered.
     *
     * @since [*next-version*]
     */
    public function testRenderSqlPrefix()
    {
        $type1 = uniqid('type-');
        $value1 = uniqid('value');
        $render1 = uniqid('render');
        ...
        $expression = $this->createExpression($prefixType, $terms);
        $ctx = [
            ExpressionContextInterface::K_EXPRESSION => $expression,
        ];
